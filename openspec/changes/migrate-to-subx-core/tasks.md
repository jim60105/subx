...
- [x] 3.2 Drop the crate name from the product description instead of swapping it (user decision: SubX implicitly uses its engine crate — same project family, the name says it; no crate belongs in product wording). Move the byte-coupled pair IN ONE step or the suite breaks: `src-tauri/tauri.conf.json` `bundle.longDescription` — "…powered by AI and subx-cli." → "…powered by AI." — and the literal it is pinned to in `scripts/version-consistency.test.ts` (asserts `bundle.longDescription` equals that string verbatim). Same sentence in `flatpak/subx.metainfo.xml`'s description paragraph (unguarded — `scripts/release-config.test.ts` reads the metainfo file but never asserts its description). Separately (different sentence, pinned by nothing — the `backendCodeParity` frontend scan walks only `src/`): `package.json` `"description": "Desktop GUI for subx-cli — AI-powered subtitle matching"` and the `description` in `src-tauri/Cargo.toml` — reword both to describe the product without any crate name (adopted: "AI-powered subtitle matching, conversion, sync, and translation desktop app", matching the pinned `bundle.shortDescription`).
...
- [x] 5.1 Update `AGENTS.md` (Project & Architecture: the app sentence describes SubX itself, not "a GUI for subx-cli" — user decision: the product is self-describing; the next paragraph keeps `subx-core` as the dependency fact and the config-store interop with the `subx-cli` binary; Cli Isolation invariant reworded to the crate-agnostic engine-crate boundary matching D3), `README.md` and `README.zh-TW.md` tagline/overview reworded the same way (companion-CLI/config-interop/Related-projects sentences keep naming `subx-cli` — those are true statements about the binary, not the dependency).
