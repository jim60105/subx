...
- Add the Convert wizard UI on top of `WizardShell` as a three-step flow:
  ...
  - **Step 2 — Options & preview**: target format (srt / ass / vtt / sub, defaulting from `formats.default_output`), keep-original toggle (GUI default ON — deleting inputs is opt-in, inverting the CLI's default), and a per-file preview of exact output paths with conflict flags (existing file would be overwritten; input already in the target format resolves to itself and is auto-excluded). Per-item checkboxes choose what runs.
