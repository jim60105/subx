...
- **THEN** the build runs inside a Flatpak sandbox on the `org.freedesktop.Platform` runtime (providing the GUI system libraries), and a pinned Rust toolchain is installed into the sandbox so `cargo build --release` succeeds; the produced bundle needs no host-level `libwebkit2gtk` or GTK installation
