...
- Product-metadata and repository-doc wording — **crate names dropped entirely** (implementer decision on record: SubX implicitly uses its engine crate; both crates belong to the same project family and the name `SubX` expresses it, so neither `subx-cli` nor `subx-core` belongs in the product description; the move away from the facade severs any remaining *dependency* claim on the CLI too). The byte-coupled pair moves in one step: `src-tauri/tauri.conf.json` `bundle.longDescription` ("…powered by AI and subx-cli." → "…powered by AI.") ↔ the literal `scripts/version-consistency.test.ts` asserts against it. The same sentence in `flatpak/subx.metainfo.xml` (unguarded). `package.json` and `src-tauri/Cargo.toml` descriptions (a different sentence — "Desktop GUI for subx-cli — …" — pinned by nothing) reworded to name no crate. `AGENTS.md`/README pair describe SubX itself; sentences that are true statements about the **CLI binary** (config-store interop at `~/.config/subx/config.toml`, companion-tool pointers, Related Projects) keep naming `subx-cli`.
