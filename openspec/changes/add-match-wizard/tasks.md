...
- [ ] 1.2 Define DTOs in `dto.rs` (+ TypeScript mirrors): `SourceScanResult`, `MatchPlanDto` (video-grouped operations with IDs, target paths, confidence, reasoning; unmatched videos/subtitles lists), `ExecutionReportDto` (from `Vec<OperationOutcome>`)
