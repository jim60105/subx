...
The release pipeline contract: single-sourcing the application version from package.json, embedding package identity metadata, generating changelog-driven release notes, running a multi-platform tag-triggered release workflow with build provenance attestations, and publishing unsigned installers for supported desktop operating systems.
...
The release workflow SHALL additionally build a Linux Flatpak bundle and attach it to the draft release as a download file. The bundle SHALL be built inside a Flatpak sandbox on the `org.gnome.Platform` 23.08 runtime plus the `org.gnome.Platform.Extension.WebKit` extension, so it carries its own WebKit/GTK runtime; a pinned Rust toolchain is installed into the build sandbox for the compile step. The bundle SHALL be published to the GitHub release rather than to Flathub. The bundle SHALL carry a build-provenance attestation like every other published file.
...
- **THEN** the build runs inside a Flatpak sandbox on the `org.gnome.Platform` runtime (providing the GUI system libraries), and a pinned Rust toolchain is installed into the sandbox so `cargo build --release` succeeds; the produced bundle needs no host-level `libwebkit2gtk` or GTK installation
