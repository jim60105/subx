...
The project's automated verification contract: an enforced minimum test coverage
for both the TypeScript and Rust sources, a mechanical traceability from every
specification scenario to the test that verifies it (or to a justified waiver),
and the CI pipeline that runs both on every push. This capability exists so that
"we test things" is an enforced fact rather than an accident, before larger
